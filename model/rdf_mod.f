...
! KWu2017: Add on for solid phase friction coefficient
!     DOUBLE PRECISION :: par_fric      
...
! Revised by KWU2017
! Torquato as used within Sundaresan
! User defined Radial Distribution Function
         CASE(USR_RDF)
!---------------------------------------------------------------------//           
	      EPS = AVG_XYZ(EP_s(IJK1, M1), EP_s(IJK2, M1), DIR, L)
            G_0AVG = G_0CS(EPS)+(0.58D0*EPS**2)/((1-EP_star_array(IJK1)-EPS)**(3/2))
         END SELECT
      ENDIF
